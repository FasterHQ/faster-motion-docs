...
| [Text Sequence Animation](text/textSequenceAnimation.md) | `textSequenceAnimation` | shared | Cycle through a sequence of strings based on progress and write the current text as the textContent of a target element. Compound: expanded into `textSequence + domPoseWrite` at load time — no runtime class. |
