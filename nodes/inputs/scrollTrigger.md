...
| `anticipatePin` | float | `0` | Pre-engage layer warmup. Set to `1` to ask the browser to promote the pin target to its own compositor layer BEFORE scroll first crosses the start edge — eliminates the one-frame paint jitter when `position: fixed` first takes effect. The hint is binary in practice (1 = on, 0 = off); larger numbers are no stronger. Internally applies `will-change: transform` on the pin target at bind and restores the original on release. (min: 0, max: 1, step: 1) |
