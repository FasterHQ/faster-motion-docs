...
| `slotStart` | float | `0` | Slot Start (playhead) (step: 0.05) |
| `slotEnd` | float | `1` | Slot End (playhead) (step: 0.05) |
| `channels` | propertyAnimationChannels | `{"opacity":{"cssUnit":"none...` | Per-CSS-property keyframe channels for this slot. Optional per-channel `template: "blur({value}px)"` wraps float output into a CSS string. Mutually exclusive with `cssUnit`. |
