...
| `invalidateOnRefresh` | bool | `false` | When `true`, the runtime re-resolves elements and re-reads every `valueFromCSS` / `colorFromCSS` keyframe whenever a page-wide layout refresh fires (window resize, fonts.ready, window.load). Lets per-element CSS variable values survive media-query swaps, theme toggles, and post-font-load reflows without an explicit re-bind. The loader emits a singleton `refreshSignal` node and wires it automatically; no manual wiring required. Default `false` — opt-in because the recompile is non-trivial for large element counts. |
